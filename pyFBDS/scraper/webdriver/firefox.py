...
class Firefox(webdriver.Firefox):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Services
        my_service = FirefoxService(executable_path=gecko_path, log_path=logs_filepath)
        ...
        my_options.set_preference("browser.download.dir", str(self.download_path))
        ...
        my_options.set_preference("browser.helperApps.showOpenOptionForPdfJS", False)
        ...
        my_options.set_preference("dom.confirm_repost.testing.always_accept", True)
    ...
    def add_extension_xpath(self) -> None:
        ...
        if not xpath_path.is_file():
            r = requests.get(url=config.URL_ADDONS_XPATH, timeout=60, verify=False)
