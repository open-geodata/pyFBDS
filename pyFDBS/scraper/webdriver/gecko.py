...
class Gecko:
    ...
    def _get_geckodriver(self, verify_ssl: bool):
        ...
        if not self.has_geckodriver:
            if platform.system() == "Windows":
                ...
                r = requests.get(
                    url=config.URL_GECKODRIVER_WINDOWS,
                    timeout=60,
                    verify=verify_ssl,
                )
                ...
                # Extract
                with ZipFile(file=gecko_zip_filepath, mode="r") as zip_ref:
                    zip_ref.extractall(self.driver_path)

            elif platform.system() == "Linux":
                # Download do geckodriver
                r = requests.get(
                    url=config.URL_GECKODRIVER_LINUX,
                    timeout=60,
                    verify=verify_ssl,
                )
                ...
                gecko_zip_filepath = self.driver_path / name_gecko
                with open(file=gecko_zip_filepath, mode="wb") as f:
                    f.write(r.content)

                # Extract
                with tarfile.open(file=gecko_zip_filepath, mode="r") as tar_ref:
    ...
    def get_path_geckodriver(self, verify_ssl: bool = False) -> Path | str:
        ...
        if platform.system() == "Windows":
            ...
            raise RuntimeError(f"Ajustar para plataforma {platform.system()}")
